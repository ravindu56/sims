...
\noindent\textbf{Screenshot:} \textit{[Insert screenshot of login page]}
...
\noindent\textbf{Screenshot:} \textit{[Insert screenshot of DevTools
$\to$ Application $\to$ Cookies, showing JSESSIONID after login]}
...
\noindent\textbf{Screenshot:} \textit{[Insert screenshot of DevTools
$\to$ Network $\to$ login POST $\to$ Response Headers, showing Set-Cookie]}
...
\noindent\textbf{Screenshot:} \textit{[Insert screenshot of View Students
page showing ``Welcome, admin'' and DevTools $\to$ Network $\to$
Request Headers showing Cookie being sent]}
...
\noindent\textbf{Screenshot:} \textit{[Insert screenshot showing that
accessing \texttt{/viewStudents} directly without login redirects to
\texttt{login.jsp}]}
...
\noindent\textbf{Screenshot:} \textit{[Insert screenshot of DevTools
$\to$ Network $\to$ logout $\to$ Response Headers showing
\texttt{Max-Age=0} cookies]}
...
\node (dec)      [decision, below of=validate, yshift=-0.5cm]  {Valid?};
\node (create)   [box, below of=dec, yshift=-0.5cm]
                                                {2. CREATION\\Session created,\\loggedInUser set};
\node (transmit) [box, below of=create]         {3. TRANSMISSION\\\texttt{Set-Cookie: sessionid=XYZ}\\HttpOnly; Max-Age=1800};
...
\node (logout)   [box, below of=filter]         {6. TERMINATION\\\texttt{session.invalidate()}\\Cookie Max-Age=0};

\node (error)    [box, right of=dec, xshift=4cm, fill=red!10, draw=red!60]
